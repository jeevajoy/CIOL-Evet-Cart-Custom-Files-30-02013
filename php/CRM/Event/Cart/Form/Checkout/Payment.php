...
class CRM_Event_Cart_Form_Checkout_Payment extends CRM_Event_Cart_Form_Cart { 
  ...
  //<-----added by Jag 10 oct 2012 ------->
  public $is_pay_later = TRUE;
  //<------end of code Jag 10 oct 2012 --->
  ...
  function buildQuickForm() {
    ...
    //<!-------added by Jag 10 oct 2012 ---->
    $membershipNumber = $this->_price_values['membershipNumber']; 

    $discount_percentage = self::get_dicsount_percentage($this->cart->events_in_carts,$membershipNumber );
    //<!-----end of code  by Jag 10 oct 2012 ---->
    //$this->add('text','email', ts('Email Address'), $textarea_size, TRUE);
    // iterate over each event in cart
    foreach ($this->cart->get_main_events_in_carts() as $event_in_cart) {
     //<!-------added by Jag 10 oct 2012 ---->
     $this->process_event_line_item($event_in_cart,$discount_percentage); 
     //<!-----end of code  by Jag 10 oct 2012 ---->
  ...
  function process_event_line_item(&$event_in_cart, $discount_percentage, $class = NULL) {
    $cost         = 0;
    //<!---- Add Discount Price Set if present added by Jag 30-01-2013 ------>    
    $discountId = CRM_Core_BAO_Discount::findSet($event_in_cart->event_id, 'civicrm_event');     
    if ($discountId) {
    ...
    //<!---- end of code added by Jag 30-01-2013 ------> 
    ...
    if ($price_set_id) {
      ...
      //print_r($event_price_values);exit;
   ...
   $this->add_line_item($event_in_cart,$discount_percentage, $class);
  }

  function add_line_item($event_in_cart, $discount_percentage, $class = NULL) {
    $amount = 0;
    $cost = 0;
    ...
    //<!-------added by Jag 10 oct 2012 ---->
    $amount =  sprintf ("%.2f",($amount - ( $amount * $discount_percentage/100))); 
    //<!-----end of code  by Jag 10 oct 2012 ----> 
  ...
  function postProcess() {
    ...
    //<!-------added by Jag 10 oct 2012 ---->
    $membershipNumber = $this->_price_values['membershipNumber']; 
    $emails = $this->_price_values['emails'];
    
    $discount_percentage = self::get_dicsount_percentage($this->cart->events_in_carts,$membershipNumber ); 
    $main_participants = $this->cart->get_main_event_participants();    
    $main_participant = end($main_participants);
    $main_contact_id = $main_participant->contact_id;
    $main_email = $main_participant->email; 
    foreach($main_participants as $participant => $value ){
       $value->email = $main_email;
       $value->contact_id = $main_contact_id;
    }
    //<!-----end of code  by Jag 10 oct 2012 ----> 
    ...
    foreach ($this->all_participants as $mer_participant) {
      ...
      if ($mer_participant->must_wait) {      
        ...
        if ($event_in_cart->event->contribution_type_id && $mer_participant->cost) {
          ...
          //<!-------added by Jag 10 oct 2012 ---->
          $params['amount'] =  ($params['amount'] - ( $params['amount']* $discount_percentage/100));
  ...
  function record_contribution(&$mer_participant, &$params, $event) {
    ...
    $contribParams = array(
      ...
      'currency' => CRM_Utils_Array::value('currencyID', $params),
      'source' => $event->title." ".$params['discount_percentage']." discounted",
   ...
   * input params # Event carts 
   * # External Identifier  
   * Returns Discount Percentage       
  */
  
  function get_dicsount_percentage(&$event_carts, $membershipNumber) {     
    ...
    $event_count =  count($event_carts);
    if($event_count > 5){
       $event_count = 5;
    }    
    $discount_percentage = array_search($event_count,$discount_array); 
     // add by jana 09/12/13
    $emails = $this->_price_values['emails'];     
    if(!empty($membershipNumber)){
        require_once 'api/api.php';
        $contact = civicrm_api("Contact","get", array ('version' => '3','sequential' =>'1', 'external_identifier'=>$membershipNumber));        
        if(!empty($contact['values'])){
            $contactID =  $contact['values'][0]['contact_id'];
             // add by jana 09/12/13 , get email id from contact
            $email=$contact['values'][0]['email'];
            $membershipDetails = civicrm_api("Membership","get", array ('version' => '3','sequential' =>'1', 'contact_id' =>$contactID));                   
            $membership = end($membershipDetails['values']);               
            if( $membership['status_id'] < 4 && $event_count > 1 ){
              if($email==$emails){
