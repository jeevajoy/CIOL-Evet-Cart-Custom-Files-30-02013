...
class CRM_Event_Cart_Form_Checkout_ParticipantsAndPrices extends CRM_Event_Cart_Form_Cart { 
  ...
  function buildQuickForm() {
    ...
    // $this->add('checkbox','event_in_cart',ts(''), null);
    //<!---------------added by Jag 08 oct 2012 ------------->
    $this->addElement('text', 'membershipNumber', ts('If you have a Membership Number, enter it here'));
    //<!----------end of code by Jag 08 oct 2012  ----------->
    // add by jana 09/12/13
    $this->addElement('text', 'emails', ts('Please enter below your Email address held by CIOL'));
  ...
  function build_price_options($event) {
    ...
    $discountId = CRM_Core_BAO_Discount::findSet($event->id, 'civicrm_event');     
    if ($discountId) {
  ...
  function validate() {
    ...
    $member_num=$fields['membershipNumber'];
  ...
  function postProcess() {
   ...
   /* // <!---added by Jag  21-01-2013-----> 
    foreach ($this->_submitValues['event'] as $event_id => $participants) {
     ...
     foreach($this->price_fields_for_event as $eKey => $eValue){
       foreach($this->cart->events_in_carts as $event_id => $values){
        if($eKey == $event_id  ) {
          foreach($values->event_cart->events_in_carts as $key => $value ){           
            foreach($value->participants as $participant_id=>$v){             
             $this->_submitValues['event'][$value->event->id]['participant'][$participant_id]['email']=$email;                               
            }             
          }
        }
      }
    }
   // <!---end of code by Jag  21-01-2013-----> */    
